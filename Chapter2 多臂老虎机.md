...
当一种学习方法应用于该问题时，如在时间步长t选择动作$`A_t`$,该动作的实际奖励$`R_t`$从均值为$`q_*(A_t)`$方差为1的正态分布中选择。图2.1灰色的部分表示奖励的分布。我们称这一组测试为10臂老虎机。对于任何一个学习方法，当其应用于一个老虎机问题时，我们可以在1000个时间步中测量该行为方法的性能和行为。这就构成了一次运行。重复运行上述操作2000次，每一次都运行在不同的臂上，我们便获得了学习算法平均行为的度量。（译者注：对于10臂老虎机来说，重复运行的操作次数为2000次，平均下来老虎机的每一个臂重复运行2000/10=200次，每一次运行1000个时间步。）
**Figure2.2**，如上所述，在10臂老虎机上比较贪婪方法和两个𝜖-贪婪方法（𝜖=0.01和𝜖=0.1）。使用样本平均法计算行动的估计价值，
