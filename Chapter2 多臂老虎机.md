...
在我们的k臂老虎机问题中，每个拉杆被选择时，都会有一个期望或平均奖励，我们将平均奖励称为该动作的价值。我们用A<sub>t</sub>表示在t时刻做的选择，选择A<sub>t</sub>的奖励用R<sub>t</sub>表示。任意操作a的价值用$`q_{\ast}(a)`$表示，$`q_{\ast}(a)`$是行动a的预期奖励：
$q_{\ast}(a) \dot{=} \mathbb{E}[R_t \mid A_t = a]$

如何你知道每个动作的价值，那么解决k臂老虎机问题将会变的非常容易：你总是选择最高价值的行动。我们假设你不确定每个行动的价值，但你知道每个动作的估计价值，我们将t时刻行动a的估计价值记为Q<sub>t</sub>(a)。我们希望Q<sub>t</sub>(a)无限接近$`q_{\ast}(a)`$。
